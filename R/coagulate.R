...
#'   containing elements `K1`, `K2`, `x1`, `x2`, `x3`, and `b`.
...
coagulate <- function(data, coefs) {
  rlang::exec(
    ...
    !!!as.list(coefs[c("K1", "K2", "x1", "x2", "x3", "b")])
  )
}

#' @rdname coagulate
#' @export
coagulate_base <- function(DOC, dose, pH, UV254, K1, K2, x1, x2, x3, b) {
  ...
  DOC_final <- (first_term + (-1) * sqrt_term) / denominator
